...
class AbilityBlockPlaceProcess {
    ...
    static void process(PreparedStatement preparedStmt, int batchCount, Material blockType, int blockData, Material replaceType, int replaceData, int forceData, String user, Object object, String newBlockData, String replacedBlockData, String player, String ability) {
        if (object instanceof BlockState) {
            ...
            if (forceData == 1) {
                AbilityBlockPlaceLogger.log(preparedStmt, batchCount, user, block, MaterialUtils.getBlockId(replaceType), replaceData, blockType, blockData, true, meta, newBlockData, replacedBlockData, player, ability);
            } else {
                AbilityBlockPlaceLogger.log(preparedStmt, batchCount, user, block, MaterialUtils.getBlockId(replaceType), replaceData, blockType, blockData, false, meta, newBlockData, replacedBlockData, player, ability);
            }
        }
    }
}
